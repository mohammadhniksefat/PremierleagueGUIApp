...
import model.database_manager  as database_manager
from model.models.base_model import BaseModel
import sqlite3, math

class SQliteModel(BaseModel):
    schemas = {
        'players' : Path(__file__).parent / ".." / "schemas" / "sqlite" / "players_table.sql", 
        'teams' : Path(__file__).parent / ".." / "schemas" / "sqlite" / "teams_table.sql",
        'matches' : Path(__file__).parent / ".." / "schemas" / "sqlite" / "matches_table.sql",
        'tables' : Path(__file__).parent / ".." / "schemas" / "sqlite" / "tables_table.sql"
    }
    ...
    def __init__(self, database_address):
        ...
        db_manager = database_manager.SQliteDatabaseManager()
    ...
    def get_required_columns_names(self):
...
class PlayersModel(SQliteModel, base_model.PlayersModel):
    def __init___(self, database_address):
        ...
        super().__init__(database_address)
